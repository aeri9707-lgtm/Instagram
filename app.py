...
with _sidebar_col:
    with st.container(key="sidebar_panel"):
        ...
        if _platform == "instagram":
            # 검색 모드
            st.markdown("<div class='sidebar-section-label'>검색 모드</div>", unsafe_allow_html=True)
            _mode_labels = {
                "search":    "🔍 크리에이터 검색",
                "following": "🏢 브랜드 역추적",
                "similar":   "🔄 유사 계정 탐색",
            ...
            _sel_mode_label = st.radio(
                ...
                label_visibility="collapsed", key="sidebar_mode_radio",
            )
            ...
            # 검색 설정
            st.markdown("<div class='sidebar-section-label'>검색 설정</div>", unsafe_allow_html=True)
        ...
        if "profiles" in st.session_state and st.session_state["profiles"]:
            ...
            st.markdown("<div class='sidebar-section-label'>결과 필터</div>", unsafe_allow_html=True)
