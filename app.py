...
# ── Apify 토큰 미설정 배너 ────────────────────────────────────────
_has_token = bool(st.session_state.get("apify_token") or os.getenv("APIFY_API_TOKEN"))
if not _has_token:
    st.markdown(
...
<script>
// parent window에 전역 함수 등록 → st.markdown onclick에서 호출 가능
window.parent.seg_click = function(key) {{
  var btns = window.parent.document.querySelectorAll('.st-key-' + key + ' button');
  if (btns.length) {{ btns[0].click(); }}
}};
function click_btn(key) {{ window.parent.seg_click(key); }}
</script>
...
if _platform == "instagram":
    if _cur_mode == "search":
        ai_limit = st.session_state.get("ai_limit", 20)
    elif _cur_mode == "following":
        fw_limit = st.session_state.get("fw_limit", 30)
    else:
        sim_limit = st.session_state.get("sim_limit", 15)
...
with st.container(key="main_panel"):
    ...
    if _platform == "tiktok":
        ...
        with tt_tab1:
            ...
            st.markdown(
                ...
                "<div style='text-align:center;color:#999;font-size:14px;'>Coming soon</div>",
                unsafe_allow_html=True)
            st.markdown("<div style='height:40px'></div>", unsafe_allow_html=True)
        with tt_tab2:
            st.markdown("<div style='height:40px'></div>", unsafe_allow_html=True)
    ...
    # ── Instagram ─────────────────────────────────────────
        ...
        _cur_mode = st.session_state["search_mode"]

        # ── 브랜드 역추적 ─────────────────────────────────
        if _cur_mode == "following":
            ...
            if st.button("🔄 유사 계정 탐색", type="primary", key="sim_btn", use_container_width=True):
                if not sim_input.strip():
                    ...
                    if err:
                        ...
                        st.dataframe(
                            sim_df, use_container_width=True,
                            column_config={
        ...
        # ── AI 크리에이터 검색 ────────────────────────────
            ...
            _CHIPS_POOL = [
                ...
                "글로우 메이크업 뷰티 크리에이터 팔로워 5만 이상",
                "비건 클린뷰티 한국 나노 인플루언서",
                "K뷰티 스킨케어 루틴 팔로워 3만~15만",
                "한국 육아 맘 팔로워 3만~10만",
                "신생아 육아 일상 엄마 마이크로인플루언서",
                "두 아이 엄마 육아템 공구 팔로워 5만 이상",
                "육아 브이로그 감성 맘 한국",
                "피트니스 다이어트 여성 팔로워 5만 이상",
                "홈트레이닝 운동 크리에이터 마이크로",
                "건강식품 영양제 리뷰 팔로워 3만 이상",
                "필라테스 요가 여성 강사 한국",
                "먹방 맛집 탐방 한국 팔로워 10만 이상",
                "홈쿡 레시피 요리 크리에이터 팔로워 5만",
                "비건 채식 요리 한국 마이크로인플루언서",
                "카페 디저트 감성 한국 나노",
                "강아지 고양이 반려동물 한국 마이크로",
                "펫푸드 간식 반려견 크리에이터 팔로워 3만",
                "고양이 일상 감성 계정 팔로워 5만 이상",
                "서울 감성 인테리어 홈카페 나노 계정",
                "미니멀 인테리어 홈스타그램 팔로워 5만",
                "신혼 인테리어 셀프 DIY 한국 마이크로",
                "데일리룩 패션 20대 여성 팔로워 3만 이상",
                "스트릿 패션 서울 남성 마이크로인플루언서",
                "국내 여행 감성 크리에이터 팔로워 5만",
                "제주 여행 일상 감성 계정 팔로워 3만",
                "재테크 주식 경제 크리에이터 팔로워 10만",
                "20대 라이프스타일 일상 브이로그 마이크로",
            ]
                    ...
                    if err:
                        ...
                        if "월간 사용량 한도" in err:
                            st.warning(
                                ...
                                "- [Apify 대시보드](https://console.apify.com/billing)에서 플랜을 업그레이드하세요.",
                                icon="⚠️",
                            )
                        else:
                            st.error(err)
                        ...
                        if _cur_query and _cur_query not in _favs_now:
                            if st.button("⭐ 이 검색어 즐겨찾기 저장", key="fav_save_btn"):
                                ...
                                st.session_state["fav_queries"] = _favs_now[:10]
                                st.rerun()
                        ...
                        if len(filtered) < 3 and f_min > 0:
                            st.info(f"팔로워 {f_min:,}명 이상 조건으로 필터하면 결과가 적을 수 있어요.")
                        _all_usernames = [p["username"] for p in profiles_raw]
                        _est_cost = estimated_precise_filter_cost(len(_all_usernames))
                        with st.expander(
                            f"🎯 정밀 팔로워 필터 — 후보 {len(_all_usernames)}개 계정 재필터 (예상 ₩{_est_cost:,})",
                            expanded=(len(filtered) < 3 and f_min > 0),
                        ):
                            st.markdown(
                                f"<div style='font-size:13px;color:#555;line-height:1.7;'>"
                                f"기본 검색은 bio 기반이라 팔로워 수가 부정확할 수 있어요.<br>"
                                f"<b>instaprism 정밀 필터</b>로 실제 팔로워 수를 확인해 "
                                f"<b>{f_min:,}~" + (f"{f_max:,}" if f_max < 999_999_999 else "∞") + "명</b> 범위만 추려줘요."
                                "</div>",
                                unsafe_allow_html=True,
                            )
                            st.caption(f"💳 계정당 약 ₩10 × {len(_all_usernames)}개 = 약 ₩{_est_cost:,}")
                            if st.button(f"🎯 정밀 필터 실행 (₩{_est_cost:,} 소모)", type="primary", key="precise_filter_btn", use_container_width=True):
                                _pf_bar = st.progress(0)
                                _pf_status = st.empty()
                                def _pf_progress(msg):
                                    _pf_status.info(f"⏳ {msg}")
                                    _pf_bar.progress(50)
                                _precise, _pf_err = precise_filter_accounts(_all_usernames, f_min, f_max, progress_callback=_pf_progress, apify_token=_apify_token)
    ...
    if "profiles" in st.session_state and st.session_state["profiles"]:
        _sb_rows = []
        for _p in st.session_state["profiles"]:
            _g = classify_group(_p["followers"])
            _sb_rows.append({
                "팔로워": _p["followers"], "카테고리": _p.get("category", ""),
                "그룹": f"그룹 {_g}", "지역": _p.get("region", "해외"),
                "도시": _p.get("city", ""),
            })
        with st.expander("⚙️ 결과 필터 & 정렬", expanded=False):
            _ff1, _ff2, _ff3 = st.columns(3, gap="medium")
            with _ff1:
                _fv = [r["팔로워"] for r in _sb_rows if r["팔로워"] > 0]
                if _fv and min(_fv) < max(_fv):
                    _frange = st.slider(
                        "팔로워 범위", min_value=min(_fv), max_value=max(_fv),
                        value=(min(_fv), max(_fv)), key="flt_frange",
                    )
                    st.caption(f"{fmt_followers(_frange[0])} ~ {fmt_followers(_frange[1])}")
                _cats = sorted({r["카테고리"] for r in _sb_rows if r["카테고리"]})
                if _cats:
                    _cat_filter = st.multiselect("카테고리", options=_cats, key="flt_cat", placeholder="전체")
            with _ff2:
                _grp_filter = st.multiselect(
                    "팔로워 그룹", options=list(GROUPS.keys()),
            ...
            with _ff3:
                _region_filter = st.radio(
                    "지역 필터", options=["전체", "한국", "해외"], horizontal=True, key="flt_region",
                )
                if _region_filter == "한국":
                    _kr_cities = sorted({r["도시"] for r in _sb_rows if r["도시"] and r["지역"] == "한국"})
                    if _kr_cities:
                        _city_filter = st.multiselect("도시", options=_kr_cities, key="flt_city", placeholder="전체")
                    else:
                        st.caption("도시 정보 있는 계정 없음")
        ...
        if "profiles" in st.session_state and st.session_state["profiles"]:
            ...
            for p in st.session_state["profiles"]:
                ...
                _all_rows.append({
                    "지역": _rg, "도시": p.get("city", ""),
                    "그룹": f"그룹 {_g}", "그룹명": get_group_label(_g),
                    "계정명": p["username"], "실명/채널명": p["full_name"],
                    "팔로워": p["followers"], "팔로워 수": fmt_followers(p["followers"]),
                    "예상 단가": estimate_cost_range(p["followers"], p.get("is_verified", False)),
                    "카테고리": p.get("category", ""),
                    "인증": "✅" if p.get("is_verified") else "",
            ...
            rows = [
                ...
                if (_region_filter == "전체" or r["지역"] == _region_filter)
                and (not _grp_filter or r["그룹"].replace("그룹 ", "") in _grp_filter)
                and _frange[0] <= r["팔로워"] <= _frange[1]
                and (not _cat_filter or r["카테고리"] in _cat_filter)
            ...
            if not rows:
                st.info("조건에 맞는 계정이 없어요. 왼쪽 필터를 조정해보세요.")
            else:
                df = pd.DataFrame(rows)
                _rbadge = {"한국": "🇰🇷 ", "해외": "🌐 ", "전체": ""}.get(active_region, "")
                st.markdown(
                    f"<p style='font-size:16px;font-weight:700;color:#111;margin-bottom:12px;'>"
                    f"{_rbadge}{label}  <span style='color:#888;font-size:13px;font-weight:400;'>{len(df)}개 계정</span></p>",
                    unsafe_allow_html=True,
                )
                ...
                for _gi, (_grp, _cnt) in enumerate(_gcounts.items()):
                    ...
                    with _gcols[_gi]:
                        st.markdown(group_summary_card(_code, _cnt, get_group_label(_code), GROUP_COLORS.get(_code, "#999")), unsafe_allow_html=True)
                st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)

                with st.expander("💾 캠페인으로 저장", expanded=False):
                    _camp_name = st.text_input("캠페인 이름", value=label or "캠페인", key="camp_name_input", placeholder="예: 2024 뷰티 캠페인 A")
                ...
                if "follow_status" in st.session_state:
                    ...
                    _ff = st.multiselect("팔로우 상태 필터", options=FOLLOW_OPTIONS, key="follow_filter", placeholder="전체")
                    if _ff:
                        df = df[df["팔로우 상태"].isin(_ff)]

                result_tab1, result_tab2, result_tab3, result_tab4 = st.tabs(["📋 전체 리스트", "✉️ 그룹별 DM", "📤 DM 발송", "📊 릴스 분석"])
                # (아래 탭 내용은 기존과 동일 — 들여쓰기만 조정)
                ...
                with result_tab1:
                    ...
                    if "팔로우 상태" in df.columns:
                        disp = ["팔로우 상태"] + disp
                    if _view == "📋 테이블":
                        st.dataframe(df[disp], use_container_width=True, hide_index=True, column_config={"프로필 URL": st.column_config.LinkColumn("프로필")})
                    else:
                        _plookup = {p["username"]: p for p in st.session_state["profiles"]}
                        _cards = [profile_card(_p, _row.get("DM 문구", ""), _row.get("예상 단가", "")) for _row in rows if (_p := _plookup.get(_row["계정명"]))]
                        if _cards:
                ...
                with result_tab2:
                    ...
                    from dm_templates import STYLES as _DM_STYLES
                    st.caption(f"현재 DM 스타일: **{_DM_STYLES.get(_cur_dm_style, '기본형')}** — 우상단 아이콘에서 변경")
                    for grp in sorted(df["그룹"].unique()):
                        code = str(grp).replace("그룹 ", "")
                        color = GROUP_COLORS.get(code, "#999")
                        grp_df = df[df["그룹"] == grp]
                        st.markdown(f"<h4 style='color:{color};'>그룹 {code} — {get_group_label(code)} ({len(grp_df)}명)</h4>", unsafe_allow_html=True)
                        ...
                        st.dataframe(grp_df[["계정명", "실명/채널명", "팔로워 수", "예상 단가", "프로필 URL"]], use_container_width=True, hide_index=True, column_config={"프로필 URL": st.column_config.LinkColumn("프로필")})
                        st.divider()
                ...
                with result_tab3:
                    if not st.session_state.get("ig_logged_in"):
                        ...
                        edited = st.data_editor(df_select, column_config={"발송": st.column_config.CheckboxColumn("발송", default=False)}, hide_index=True, use_container_width=True, key=f"dm_select_editor_{select_all}")
                        ...
                        with btn_col1:
                            if st.button(f"📤 {len(selected)}개 DM 발송", type="primary", disabled=len(selected) == 0, use_container_width=True):
                                targets = [{"username": r["계정명"], "dm_text": r["DM 문구"]} for _, r in selected.iterrows()]
                        ...
                        with btn_col2:
                            if st.button(f"➕ {len(selected)}개 팔로우", disabled=len(selected) == 0, use_container_width=True):
